...
sys.path.append('../')
from data import BaseTransform, VOC_300,VOC_320, VOC_512, COCO_300,COCO_320, COCO_512, COCO_mobile_300
from data import VOC_CLASSES as labelmap
from layers.functions import Detect, PriorBox
from utils.timer import Timer

parser = argparse.ArgumentParser(description='network deploy')
...
parser.add_argument('-m', '--trained_model',
                    default=os.path.join('../weights','Atinypelee','SSD_Atinypelee_VOC_320','20190114','SSD_Atinypelee_VOC_epoches_680.pth'),
                    type=str, help='Trained state_dict file path to open')
parser.add_argument('--video', default=False, type=bool,
                    help='Video(True) or Image(False)')
parser.add_argument('--eval_file', default=None, type=str,
                    help='file path to eval')
...
if __name__ == '__main__':
    ...
    state_dict = torch.load(args.trained_model, map_location=lambda storage, loc: storage)#map GPU to CPU
    ...
    if not os.path.exists(args.eval_file):
        print('%s multimedia file not exist!'%args.eval_file)
    else:
        if args.video:
            cap = cv2.VideoCapture(args.eval_file)   #Open the built-in camera of computer.You can assign the video file
            while True:
                ret, image = cap.read()
                        ...
                        for i in range(class_collection.shape[0]):
                            if class_collection[i, -1] > 0.6:
                                ...
                                cv2.putText(image, labelmap[class_id], (int(pt[0]), int(pt[1])), FONT,
                                            0.5, (255, 255, 255), 2)
                cv2.imwrite(args.save_path + '.jpg', image)
        else:
            image = cv2.imread(args.eval_file)
            detect_bboxes = object_detector.predict(image)
            for class_id,class_collection in enumerate(detect_bboxes):
                if len(class_collection)>0:
                    for i in range(class_collection.shape[0]):
                        if class_collection[i,-1]>0.6:  #thresh = 0.6
                            pt = class_collection[i]
                            cv2.rectangle(image, (int(pt[0]), int(pt[1])), (int(pt[2]),
                                                                            int(pt[3])), COLORS[i % 3], 2)
                            cv2.putText(image, labelmap[class_id] + '%.2f'%(class_collection[i,-1]), (int(pt[0]), int(pt[1])), FONT,
                                        0.5, (255, 255, 255), 2)
            cv2.imwrite(args.save_path + '.jpg',image)
