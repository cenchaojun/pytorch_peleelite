...
class SSD(nn.Module):
    ...
    def __init__(self, num_classes, size, base, head):
        ...
        self.first_conv = nn.Conv2d(288, 128, 1, 1, 0)  # change the First channel

        self.downSampleLeft1 = DownSampleLeft()
        self.downSampleLeft2 = DownSampleLeft()
        if self.size == 300:
            self.downSampleLeft3 = DownSampleLeft(pad=1)
        else:
    ...
    def forward(self, x, test=False):
        ...
        # Second = self.downSampleLeft1(First)

        Third = torch.cat((self.downSampleLeft2(Second), self.downSampleRight2(Second)), 1)
        # Third = self.downSampleLeft2(Second)

        Fourth = torch.cat((self.downSampleLeft3(Third), self.downSampleRight3(Third)), 1)
        # Fourth = self.downSampleLeft3(Third)

        Fifth = torch.cat((self.downSampleLeft4(Fourth), self.downSampleRight4(Fourth)), 1)
        # Fifth = self.downSampleLeft4(Fourth)

        Sixth = torch.cat((self.downSampleLeft5(Fifth), self.downSampleRight5(Fifth)), 1)
        # Sixth = self.downSampleLeft5(Fifth)

        Fifth_out = self.up_relu5(self.up_convdw6(F.interpolate(Sixth, size=Fifth.size()[2], mode='bilinear')) + Fifth)

        Fourth_out = self.up_relu4(
            self.up_convdw5(F.interpolate(Fifth_out, size=Fourth.size()[2], mode='bilinear')) + Fourth)

        Third_out = self.up_relu3(
            self.up_convdw4(F.interpolate(Fourth_out, size=Third.size()[2], mode='bilinear')) + Third)

        Second_out = self.up_relu2(
            self.up_convdw3(F.interpolate(Third_out, size=Second.size()[2], mode='bilinear')) + Second)

        First_out = self.up_relu1(self.Second_up_conv(
            self.up_convdw2(F.interpolate(Second_out, size=First.size()[2], mode='bilinear'))) + First)
...
if __name__ == '__main__':
    ...
    torch.save(ssd_net.state_dict(), 'ALOD.pth')
'''
