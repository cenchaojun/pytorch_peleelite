...
class DenseBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.cat((x,cb1_b_out,cb2_c_out),1) #concate channel
...
class PeleeNet(nn.Module):
    def __init__(self,num_classes=21, num_init_features=32,growthRate=32, nDenseBlocks = [3,4,8,6], bottleneck_width=[1,2,4,4]):
...
if __name__ == '__main__':
    p = PeleeNet(num_classes=21)
    input = torch.ones(1, 3, 320, 320)
    print(p)
    output = p(input)

    print('Final FetureMap',output.size())
