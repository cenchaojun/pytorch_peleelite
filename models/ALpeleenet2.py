...
class StemBlock(nn.Module):
    def __init__(self, inp=3, num_init_features=64):
        super(StemBlock, self).__init__()

        self.stem_1 = Conv_bn_relu(inp, num_init_features, 3, 2, 1)

        self.stem_2 = nn.Conv2d(num_init_features, num_init_features, 1, 1, 0)

        self.stem_3 = ConvDW_bn_relu(num_init_features, 3, 1, 1)

        self.stem_4 = nn.Conv2d(num_init_features, num_init_features*2, 1, 1, 0)

        self.stem_5 = ConvDW_bn_relu(num_init_features*2, 3, 1, 1)
...
class DenseBlock(nn.Module):
    def __init__(self, inp, inter_channel, growth_rate):
        ...
        self.cb_b = ConvDW_bn_relu(inter_channel, 3, 1, 1)
        self.cb_c = Conv_bn_relu(inter_channel, growth_rate, 1, 1, 0)
        self.cb_d = ConvDW_bn_relu(growth_rate, 3, 1, 1)

    def forward(self, x):
        cb_a_out = self.cb_a(x)
        cb_b_out = self.cb_b(cb_a_out)
        cb_c_out = self.cb_c(cb_b_out)
        cb_d_out = self.cb_d(cb_c_out)
        
        out = torch.cat((x,cb_c_out,cb_d_out),1)
...
class PeleeNet(nn.Module):
    def __init__(self, num_init_features=64, growthRate=32, nDenseBlocks = [4,6,6,6], bottleneck_width=[2,3,4,5]):
        ...
        for i, b_w in enumerate(bottleneck_width):
            ...
            if i == 0:
                total_filter.append(self.num_init_features*2 + growthRate * nDenseBlocks[i])
