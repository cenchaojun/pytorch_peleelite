...
class StemBlock(nn.Module):
    def __init__(self, inp=3):
        ...
        self.stem_2 = nn.Conv2d(64, 64, 1, 1, 0)

        self.stem_3 = ConvDW_bn_relu(64, 3, 1, 1)

        self.stem_4 = nn.Conv2d(64, 128, 1, 1, 0)

        self.stem_5 = ConvDW_bn_relu(128, 3, 1, 1)
...
class DenseBlock(nn.Module):
    def __init__(self, inp, inter_channel):
        ...
        self.cb1_b = Conv_bn_relu(inter_channel, inter_channel, 3, 1, 2, 2)

        self.cb2_a = Conv_bn_relu(inp,inter_channel, 1, 1, 0, use_relu = False)
        self.cb2_b = ConvDW_bn_relu(inter_channel, 3, 1, 1)
        self.cb2_c = Conv_bn_relu(inter_channel, inter_channel, 1, 1, 0, use_relu = False)
        self.cb2_d = ConvDW_bn_relu(inter_channel, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        cb2_d_out = self.cb2_d(cb2_c_out)

        out = torch.cat((x,cb1_b_out,cb2_d_out),1)
